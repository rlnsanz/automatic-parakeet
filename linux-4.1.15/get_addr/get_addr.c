...
asmlinkage long sys_get_addr(int pid, unsigned long long addr)
{
	pgd_t *pgd;
	pmd_t *pmd;
	pte_t *ptep, pte;
	...
	pte = *ptep;
	if (is_swap_pte(pte)) {
		printk(KERN_INFO "Page frame is in swap. Swap Id: %llu\n", (pte_val(pte) >> 32));
		pte_unmap(ptep); 
		return 0;
	}
	page = pte_page(pte);
	if (page) {
		printk(KERN_INFO "Page frame located at 0x%llx\n", page_to_phys(page));
		pte_unmap(ptep); 
		return 0;
	}
